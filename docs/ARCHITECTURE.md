...
│   │                  FILE_WRITE, denylist bash regex, GATED delegate_task/mcp_call + semua nama bertitik,
...
```
PermissionHandler (denylist+jail realpath+cwd, allow-all tetap jail) → validateArgs (kernel) → executor (order-preserving, cap di src/constants.ts, antrean abort-aware, file-lock resolve+toLowerCase) → tool defense-in-depth → execute
...
3. **Bash security = blocklist, bukan sandbox.** Denylist regex bisa dilewati indirection variabel (`X=.env; cat $X`), quote-splitting (`cat .e""nv`), `--eval`, `env`, exfiltrasi `curl -d @.env`. `--sandbox os` (bubblewrap/seatbelt) & `--sandbox docker` tersedia tapi masih opt-in, dan `os` tidak ada di win32. → Fase 2.1 menjadikan `os` default bila tersedia.
4. **`background:true` tidak bersandbox** — ditolak eksplisit saat `--sandbox` aktif, karena container/namespace ephemeral mati bersama call-nya. Janji isolasi tidak bisa dipenuhi untuk proses yang hidup melewati turn.
5. **Git read-only** (`git_status/diff/log`); checkpoint memakai snapshot file (cap `WORKSPACE_SNAPSHOT_LIMIT`) + `spawnSync git status`, bukan shadow-git. → Fase 3.1, 4.2.
6. **`src/repo/tree-sitter.ts` adalah stub** yang selalu `return null`; repo-map nyata berbasis regex. → Fase 3.2 (implementasi atau hapus).
7. Auth hanya API key mentah — belum ada OAuth/device-code untuk provider dengan free tier. → Fase 4.1.
8. `vector.db` `LIMIT 500` — paging belum untuk memory >5k. `open()` SQLite masih sinkron per operasi (singleton pool = refactor berikutnya).
9. Benchmark `bench/runner.ts` butuh provider ber-API-key untuk hasil nyata (`--fake` hanya smoke).
10. `vendor/minicore` adalah salinan — mengeditnya langsung akan hilang saat `vendor:minicore`. Sumber kebenaran tetap repo `minicore`; publish ke npm masih opsi terbuka.
11. Lisensi **MIT** (lihat `LICENSE`) — dokumen lama menyebut EULA/proprietary, itu sudah tidak berlaku.
