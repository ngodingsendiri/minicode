...
export async function runSetupWizard(): Promise<boolean> {
  ...
  const options = [...GATEWAY_PRESETS.map((p) => ({ name: p.label, url: p.baseUrl, hint: "" })), ...EXTRA, { name: "Custom URL", url: "", hint: "Any OpenAI-compatible endpoint" }];
  options.forEach((p, idx) => {
    process.stdout.write(`  ${idx + 1}. ${p.name}${p.hint ? ` (${p.hint})` : ""}\n`);
  });
  process.stdout.write("\n");

  const choice = await ask("Choice [1-7, default 1]: ");
  const choiceNum = parseInt(choice, 10);
  const selected = (choiceNum >= 1 && choiceNum <= options.length) ? options[choiceNum - 1]! : options[0]!;
  ...
  try {
    ...
    const preset = GATEWAY_PRESETS.find((p) => p.baseUrl.replace(/\/+$/, "") === targetUrl.replace(/\/+$/, ""));
    const fallbackModels = preset?.fallbackModels ?? (targetUrl.includes("anthropic") ? ["claude-sonnet-4"] : ["gpt-4o-mini"]);
    const entry = await detectAndSave(targetUrl, apiKey, undefined, { fallbackModels });
    process.stdout.write(`[OK] Provider "${entry.id}" saved — ${entry.models.length} models detected\n`);
