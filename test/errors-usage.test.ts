...
import { friendlyFromCategory, friendlyError } from "../cli/errors.ts";
...
test("friendlyFromCategory: auth + balance", () => {
  const f = friendlyFromCategory("auth", 'credits: {"message":"Insufficient balance. Manage billing here: https://opencode.ai/workspace/wrk_123/billing"}');
...
test("friendlyFromCategory: invalid_request & context_length", () => {
  expect(friendlyFromCategory("invalid_request", "400 model not found").message).toContain("bad model");
...
test("usage: effective-model event changes cost basis", () => {
  ...
  bus.emit({ type: "provider:extension", kind: "effective-model", data: { requested: "gpt-4o", effective: "deepseek-chat", provider: "fallback-x" } });
  bus.emit({ type: "provider:extension", kind: "usage", data: { inputTokens: 1_000_000, outputTokens: 1_000_000, totalTokens: 2_000_000 } });
...
test("usage: reset clears effective model", () => {
  const bus = createEventBus();
  const collector = createUsageCollector(bus, "gpt-4o");
  bus.emit({ type: "provider:extension", kind: "effective-model", data: { requested: "x", effective: "y", provider: "z" } });
  collector.reset();
