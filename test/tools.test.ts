...
test("permission auto", async () => {
  ...
  expect(await h.check({ id: "1", name: "read_file", args: { path: "src/tools/read_file.ts" } } as any, {} as any)).toBe("allow");
  expect(await h.check({ id: "1", name: "bash", args: { cmd: "rm -rf /" } } as any, {} as any)).toBe("deny");
  expect(await h.check({ id: "1", name: "bash", args: { cmd: "echo hi" } } as any, {} as any)).toBe("allow");
  expect(await h.check({ id: "1", name: "write_file", args: { path: "../outside.txt", content: "x" } } as any, {} as any)).toBe("deny");
});

test("permission readonly", async () => {
  const h = createPermissionHandler({ mode: "readonly" });
  expect(await h.check({ id: "1", name: "read_file", args: { path: "a" } } as any, {} as any)).toBe("allow");
  expect(await h.check({ id: "1", name: "bash", args: { cmd: "echo hi" } } as any, {} as any)).toBe("deny");
});
