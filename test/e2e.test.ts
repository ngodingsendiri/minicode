...
import { FakeProvider, allowAll, text, finish, toolCall } from "../../minicore/test/fakes.ts";
...
test("e2e: full session — agent writes a file via tool, history committed", async () => {
  ...
  const provider = new FakeProvider([
    ...
    { events: [toolCall("write_file", { path: `${tmp}/out.txt`, content: "hello e2e" }), finish("tool_calls")] },
  ...
  expect(history.some((m: any) => m.role === "tool" && m.toolCallId && m.name === "write_file")).toBe(true);
