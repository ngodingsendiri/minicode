...
import { decodeKeys, applyKey, createState, buildRenderSpec, type PromptState } from "../cli/prompt-engine.ts";
...
test("fuzz applyKey: 2000 langkah acak — state valid & tidak throw", () => {
  const cmds = ["/help", "/providers", "/model", "/models", "/sync", "/undo", "/cost", "/status", "/resume", "/exit", "/review", "/fix", "/repo", "/debug"];
  const hints = (l: string) => (l.startsWith("/") ? cmds.filter((c) => c.startsWith(l)) : []);
  const keyTypes = ["char", "backspace", "up", "down", "tab", "enter", "esc", "ctrl-c", "left", "right"] as const;
  const chars = ["/", "h", "e", "l", "p", "p", "r", "o", "v", "i", "d", "e", "r", "s", "m", "a", "0", " ", "😀", "✓"];
  const rng = (n: number) => Math.floor(Math.random() * n);
  ...
  for (let step = 0; step < 2000; step++) {
    ...
    const key =
      t === "char"
        ? { type: "char", ch: chars[rng(chars.length)]! }
        : { type: t };
    ...
    buildRenderSpec(state, "minicode❯ ", hints(state.line), (t) => (cmds.includes(t.split(" ")[0]!) ? "commands" : "skills"));
  }
});
