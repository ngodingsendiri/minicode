...
beforeAll(async () => {
  ...
  await writeFile(`${dir}/package.json`, '{"name":"extreme","type":"module","scripts":{"typecheck":"bun x tsc --noEmit","test":"bun test"}}');
  await writeFile(`${dir}/tsconfig.json`, '{"compilerOptions":{"target":"ES2022","module":"ESNext","moduleResolution":"bundler","strict":true,"noEmit":true,"allowImportingTsExtensions":true},"include":["src","test"]}');
  ...
  const orFree = cfg.providers.find((p) => p.id === "openrouter" && p.models.includes("nvidia/nemotron-3-ultra-550b-a55b:free"));

  const { createOpenAICompatProvider } = await import("../../minicore/src/providers/openai-compat.ts");
  if (bai && bai.apiKey) {
    provider = createOpenAICompatProvider({ baseUrl: bai.baseUrl, apiKey: bai.apiKey, models: ["deepseek-v4-flash"], defaultModel: "deepseek-v4-flash" });
    process.env._TEST_MODEL = "deepseek-v4-flash";
    console.log("Using provider: bai / deepseek-v4-flash");
  } else if (orFree && orFree.apiKey) {
    provider = createOpenAICompatProvider({ baseUrl: orFree.baseUrl, apiKey: orFree.apiKey, models: ["nvidia/nemotron-3-ultra-550b-a55b:free"], defaultModel: "nvidia/nemotron-3-ultra-550b-a55b:free" });
