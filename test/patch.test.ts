...
test("apply_patch: replaces single block", async () => {
  ...
  const out = await applyPatchTool.execute(
    { path: `${tmp}/a.ts`, patches: [{ search: "const x = 1;\nconsole.log(x);", replace: "const x = 2;\nconsole.log(x * 2);" }] },
    ctx,
  );
...
test("apply_patch: error when search not found", async () => {
  ...
  await expect(applyPatchTool.execute({ path: `${tmp}/c.ts`, patches: [{ search: "nonexistent", replace: "x" }] }, ctx)).rejects.toThrow(/not found/);
  await rm(tmp, { recursive: true, force: true });
});
