...
function log(msg: string) { console.log(msg); try { appendFileSync(LOG, msg.replace(/\x1b\[[0-9;]*[a-zA-Z]/g, "") + "\n"); } catch {} }
...
log(`  [config] bai: ${mask(bai?.apiKey ?? "")} @ ${bai?.baseUrl}  (${bai?.models.length} models)\n  [ok] provider siap.`);
await sleep(600);
...
const session = await createMinicodeSession({ provider: router, tools: allTools, cwd: WORKDIR, permissionMode: "auto", timeoutMs: 240_000 });
...
function friendly(raw: string): string {
  ...
  if (lower.includes("auth") && /401|403/.test(raw)) return "Authentication rejected (401/403) — periksa API key.";
  if (lower.includes("rate") || /429/.test(raw)) return "Rate limited oleh provider (429) — tunggu sebentar lalu coba lagi.";
  if (lower.includes("no endpoints available")) return "Provider menolak model ini (guardrail/data policy) — coba model/produsen lain.";
  if (lower.includes("context length") || lower.includes("context_length")) return "Konteks penuh (context window) — sesi baru / model lebih besar.";
  if (lower.includes("no provider configured")) return "Tidak ada provider terpasang — jalankan `minicode config add` atau wizard.";
...
await prompt("Buat aplikasi sederhana: buat file calculator.ts dengan fungsi tambah(a,b) dan kurang(a,b), lalu buat juga calculator.test.ts yang menguji keduanya dengan bun:test", "A");
await sleep(1500);

// ── STEP 4: review/jalankan ──
await prompt("Sekarang jalankan bun test untuk file test tadi. Kalau ada error, perbaiki sampai lulus.", "B");
await sleep(1500);
