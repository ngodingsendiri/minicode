...
function run(
  label: string,
  args: string[],
  opts: { timeout?: number; expect?: RegExp; showLast?: number } = {},
): void {
  console.log(`\n── ${label} ──\n$ bun ${CLI} ${args.join(" ")}`)
...
run("2+2", ["2+2 berapa? jawab angka saja", "--model", "bai::deepseek-v4-flash"], {
  expect: /\d/,
  showLast: 5,
})
...
run(
  "substitusi",
  ["1+1 berapa? jawab angka saja", "--model", "openrouter::nvidia/nemotron-3-ultra-550b-a55b:free"],
  { expect: /via|tokens|\d/, showLast: 5 },
)
...
run(
  "write sum.ts",
  [
    "Buat file sum.ts: export function tambah(a: number, b: number): number { return a + b; }",
    "--model",
    "bai::deepseek-v4-flash",
    "--timeout",
    "180000",
  ],
  { expect: /sum\.ts|tambah|tokens/, showLast: 10 },
)
console.log(
  `\n  file check: ${existsSync("sum.ts") ? "[OK] sum.ts terbuat" : "[FAIL] sum.ts tidak ada"}`,
)
if (existsSync("sum.ts")) {
  try {
    console.log(execSync("type sum.ts", { encoding: "utf8" }))
  } catch {}
}
rmSync("sum.ts", { force: true })
...
run("sessions list", ["sessions", "list"], {
  expect: /Session ID|Recent|\(no sessions\)/,
  showLast: 8,
})
...
run("config add tanpa apiKey", ["config", "add", "--baseUrl", "https://x.ai/v1"], {
  expect: /apiKey|usage:/,
  showLast: 4,
})
