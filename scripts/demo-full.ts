...
const mic = (t: string, opts: { expect?: RegExp; timeout?: number; showLast?: number } = {}) => {
  ...
  if (ok) {
    ...
    fail++
    console.log(`  [FAIL] (mengharapkan ${opts.expect})\n  ---\n${out.slice(0, 800)}`)
  }
  return out
}
...
console.log(
  `  .minicode/config.json ada? ${existsSync(".minicode/config.json") ? "ya (provider terpasang)" : "belum (jalankan minicode config add)"}`,
)
...
mic(`"2+2 berapa? jawab angka saja" --model ${BAI}`, {
  expect: /\d/,
  showLast: 5,
  timeout: 180_000,
})
...
mic(
  `"Buat file demo-app.ts berisi fungsi hitung(a,b) yang menjumlahkan, lalu jalankan bun demo-app.ts dengan node?" --model ${BAI} --timeout 240000`,
  { expect: /demo-app|tokens|✓/i, showLast: 12, timeout: 260_000 },
)
console.log(
  `  file?: ${existsSync("demo-app.ts") ? "[OK] dibikin" : "(agent hanya pakai tool lain)"}`,
)

banner("STEP 5 — CHECKPOINT / UNDO / REDO (kolektor sesi + undo)")
mic(`"Tulis file undo-demo.ts: export const awal = 1;" --model ${BAI} --timeout 200000`, {
  expect: /tokens|undo-demo|✓/i,
  showLast: 8,
  timeout: 220_000,
})
mic("sessions list", { expect: /session|Recent/i, showLast: 6 })
...
mic(`"1+1?" --model openrouter::nvidia/nemotron-3-ultra-550b-a55b:free --timeout 120000`, {
  expect: /tokens|\d/,
  showLast: 5,
  timeout: 140_000,
})
