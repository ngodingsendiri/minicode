...
class McpConnection {
  ...
  async connect(): Promise<void> {
    ...
    try {
      const disc = await this.transport.request("server/discover", {
        _meta: {
  ...
  async callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
    const result = await this.transport.request("tools/call", { name, arguments: args }) as Record<string, unknown>;
  ...
  wrapTools(serverId: string): Tool[] {
    return this.tools.map((t) => ({
      ...
      parameters: (t.inputSchema ?? { type: "object", properties: {}, additionalProperties: true }) as Tool["parameters"],
