...
export const applyPatchTool: Tool = {
  name: "apply_patch",
  description: "Apply SEARCH/REPLACE block(s) to file. Each search must match exactly once. Supports multiple patches in one call.",
  parameters: {
    type: "object",
    properties: {
      ...
      patches: {
        ...
        items: {
          type: "object",
          properties: {
            search: { type: "string", description: "blok kode yang akan diganti (match persis atau fuzzy)" },
  ...
  async execute({ path, patches }, ctx) {
    ...
    for (let i = 0; i < patchList.length; i++) {
      ...
      if (match.mode === "exact" || match.mode === "crlf") {
        ...
        if (second && (second.mode === "exact" || second.mode === "crlf")) {
          throw new Error(`patch[${i}]: search block found multiple times in ${p} — provide more context`);
        }
      }
    ...
    if (content.length > 5_000_000) throw new Error(`result too large: ${content.length} chars (max 5M)`);
