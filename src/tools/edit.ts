...
// Cari needle dengan toleransi bertingkat:
// 1. Exact match
// 2. CRLF vs LF match
...
export function flexibleMatch(content: string, needle: string): MatchResult | null {
  ...
  if (trimStart !== -1) {
    ...
    if (lineCountBefore + needleLineCount <= cLfLines.length) {
      ...
      const startInLf = cLfLines.slice(0, lineCountBefore).join("\n").length + (lineCountBefore > 0 ? 1 : 0);
...
export const editTool: Tool = {
  name: "edit",
  description: "Edit file dengan replacement string. oldString harus ada tepat sekali (didukung fuzzy tolerance untuk spasi/indentasi). newString menggantikannya.",
  parameters: {
    type: "object",
    properties: {
      path: { type: "string" },
