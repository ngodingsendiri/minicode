...
class LspConnection {
  ...
  async stop(): Promise<void> {
    ...
    await new Promise<void>((r) => {
      const p = this.proc!;
      const t = setTimeout(() => { try { p.kill("SIGKILL"); } catch {} r(); }, 2_000);
  ...
  private dispatch(msg: Record<string, unknown>) {
    ...
    if (msg.id != null && ("result" in msg || "error" in msg)) {
      ...
      if (p) {
        ...
        if (msg.error) p.reject(new Error(String((msg.error as Record<string, unknown>).message ?? "LSP error")));
    ...
    if (msg.method === "textDocument/publishDiagnostics") {
      const params = msg.params as { uri?: string; diagnostics?: Record<string, unknown>[] } | undefined;
...
export async function lspDiagnostics(absPath: string, text: string, timeoutMs = 5_000): Promise<{ uri: string; items: Record<string, unknown>[] }> {
...
export async function lspCall(absPath: string, text: string, method: string, params: Record<string, unknown>, waitDiagsMs = 3_000): Promise<unknown> {
  const conn = await getConnection(absPath);
  const uri = conn.openDocument(absPath, text);
