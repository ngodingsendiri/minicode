...
async function embedTexts(baseUrl: string, apiKey: string, texts: string[]): Promise<number[][] | null> {
  ...
  for (const headers of headersList) {
    for (const url of urls) {
      try {
        const res = await fetch(url, { method: "POST", headers: headers as Record<string, string>, body, signal: AbortSignal.timeout(5000) });
        if (!res.ok) continue;
...
export async function searchHybrid(query: string, opts: { baseUrl?: string; apiKey?: string; cwd?: string; topK?: number } = {}): Promise<{ text: string; score: number }[]> {
  const db = open(opts.cwd);
  const rows = db.prepare("SELECT text, embedding FROM memory ORDER BY created_at DESC LIMIT 200").all() as { text: string; embedding: Buffer | null }[];
  db.close();
